...
test('audio backend transfers MP3 bytes, rejects errors, and keeps the server alive after a broken stream', async () => {
  const originalFetch = globalThis.fetch;
  audioBackendService.setConfig({ enabled: true, url: 'https://audio-provider.test' });
  const server = app.listen(0, '127.0.0.1');
  await new Promise<void>(resolve => server.once('listening', resolve));
  const { port } = server.address() as { port: number };
  let mode = 'range';
  globalThis.fetch = (async (url: any, options: any) => {
    assert.equal(String(url), 'https://audio-provider.test/audio/abc-def_123'); // No hidden fallback to InnerTube.
    ...
    if (mode === 'range') {
      ...
      return new Response(new Uint8Array([1,2,3,4]), { headers: { 'content-type': 'audio/mpeg', 'content-length': '4' } });
    }
  ...
  try {
    ...
    for (const [failure, message] of [['empty', /empty audio stream/], ['html', /non-MP3/], ['media-error', /HTTP 500/]] as const) {
      mode = failure;
